...
\newglossaryentry{jet}{
        name=jet,
        description={A collimated shower of hadronic particles. High momentum quarks and gluons fragment due to colour confinement; the resulting particles combine into hadrons and deposit energy in the detector very close to each other and is reconstructed as a single physics object called a jet}
}
...
\newglossaryentry{antikt}{
        ...
        description={A sequential clustering algorithm designed to group hadronic particles into jets with a radius parameter ($R$). The transverse momentum is symbolised as \kt instead of \pt. In CMS, standard radius parameters are $R = 0.4$ and $R = 0.8$, referred to as AK4 and AK8 jets, respectively},
        symbol={anti-\kt}
}
...
\newglossaryentry{met}{
        ...
        description={The negative vector sum of the transverse momentum of all particles in a collider event. It is sometimes abbreviated to ``MET'' with the symbol $\etmiss$, and also referred to in literature as ``missing transverse momentum'' ($\ptmiss$)},
        symbol={$\etmiss$}
}
