...
The preselection is defined by applying the following cuts:
\medskip
\begin{easylist}[itemize]
    \cutflowlistprops
    & $\ptjone > \text{80} \GeV$
    ...
    & $\mht > \text{200} \GeV$
    & $\MET > \text{200} \GeV$
    & $\mht/\MET < \text{1.2}$ (inverted only for the QCD-enriched sidebands)
...
Further selections are applied to all years, regions and categories to filter poorly measured or misreconstructed events in both data and MC. A ``muon jet filter" rejects events with misreconstructed muons by requiring all jets with $\pt > \text{200}\GeV$ must have a muon energy fraction $> \text{0.5}$, and $\Delta\phi(\mathrm{j}, \ \ptmiss) > \pi - \text{0.4}$.

Charged (CHF) and neutral hadron energy fraction (NHF) requirements are applied to all jets via fulfillment of the ``tight" jet ID criteria (see [SEC IN OBJECT DEFS]). Furthermore, stricter requirements are placed on the leading two jets---in all categories, except the monojet categories where the requirements only apply to the leading jet---as follows:

\medskip

\begin{easylist}[itemize]
    \cutflowlistprops
    & $\text{CHF}_{\jone} > \text{0.1}$
    & $\text{NHF}_{\jone} < \text{0.8}$
    & $\text{CHF}_{\jtwo} > \text{0.1}$
    & $\text{NHF}_{\jtwo} < \text{0.8}$
\end{easylist}

\medskip

\noindent{}Events with anomalously energetic jets have been observed for data in the HF between $\abs{\eta} = [3.0, 3.1]$ that can lead to excess events, particularly in the QCD-enriched sidebands. As such, a requirement is placed on the ratio of the \HT formed from jets within $\abs{\eta} < \text{5}$ ($\HT^5$) and $\abs{\eta} < \text{2.4}$ ($\HT^{2.4}$), in OR with the difference in $\phi$ between the leading jet and \mht:
\medskip
\begin{easylist}[itemize]
    \cutflowlistprops
    & $\dfrac{\HT^5}{\HT^{2.4}} < \text{1.2}$ \ or \ $\Delta\phi(\jone, \mht) \geq \left( \text{5.3} \frac{\HT^5}{\HT^{2.4}} - \text{4.78} \right)$
\end{easylist}

\medskip

\noindent{}where, if the former requirement is not satisfied, the latter is constructed such that $\Delta\phi(\jone, \mht) \geq \sfrac{\pi}{2}$. The \HT variable in the preselection, and in any plots showcased, only have requirements on the $\abs{\eta}$ for the two hardest jets---from the VBF orthogonality constraints in Chpt.~\ref{subsec:htoinv_preselection}.

To avoid any potential horns (spikes in data and/or \acrshort{mc}) for jets at large pseudorapidity, we require
\medskip
\begin{easylist}[itemize]
    \cutflowlistprops
...
The filters described below were recommended to remove events with miscalculated \ptmiss:
\medskip
\begin{easylist}[itemize]
    \easylistprops
    ...
    & Filter for dead cells when constructing trigger primitives in the \acrshort{ecal}
    & Filter for low-quality \gls{pf} muons
\end{easylist}
...
Note that while the signal region and QCD sidebands have hadronic final states, the implementation of veto weights for leptons in Sec.~\ref{subsubsec:veto_sel_weights} does not automatically reject events containing leptons. To avoid ambiguity and the inclusion of potentially misreconstructed events, an explicit veto is placed for the electrons satisfying the above criteria.
...
% Add trigger selection as part of region definitions.
...
% Mention that the leptons/photons are removed from the MET calculation. And, obviously control regions are constructed to be, ideally, devoid of signal.
