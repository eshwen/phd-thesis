...
\medskip
% Mention that the leptons/photons are removed from the MET calculation to simulate the processes they are predicting (single lepton region predicts semi-leptonic ttbar and Wlnu, in either case where the lepton is lost, double lepton and photon regions predict Znunu, the former parallels the decay to 2 leptons as kinematics are very similar, and the latter substitutes the Z->nunu for a photon(?))
...
For the \singlePhotonCr, we take data from \acrshort{cms} originating only from the dataset of \Pphoton-based triggers. Data and \acrshort{mc} must satisfy any of the triggers from Tab.~\ref{tab:htoinv_photon_pd_triggers} for the respective year.
...
\end{table}
% See if I can describe them in a more elegant way. Might be easier give the descriptions and actual HLT path names a bit earlier, then just reference the HLT path names in the list
...
% Based on kinematic requirements. Mention that the sidebands which only invert one variable are kinematically similar to the signal region. So, when conducting an analysis while blind to data in the SR, inspecting these sidebands is a good indicator of, how, e.g., new cuts or corrections/weights, will affect the SR and data-MC agreement.
