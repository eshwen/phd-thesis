...
Further selections are applied to all years, regions and categories to filter poorly measured or misreconstructed events in both data and MC. A ``muon jet filter" rejects events with misreconstructed muons by requiring all \glspl{jet} with $\pt > \text{200}\GeV$ must have a muon energy fraction $> \text{0.5}$, and $\Delta\phi(\mathrm{j}, \ \ptmiss) > \pi - \text{0.4}$.

Charged (CHF) and neutral hadron energy fraction (NHF) requirements are applied to all \glspl{jet} via fulfillment of the ``tight" jet ID criteria (see [SEC IN OBJECT DEFS]). Furthermore, stricter requirements are placed on the leading two \glspl{jet}---in all categories, except the monojet categories where the requirements only apply to the leading jet---as follows:
...
\noindent{}Events with anomalously energetic \glspl{jet} have been observed for data in the HF between $\abs{\eta} = [3.0, 3.1]$ that can lead to excess events, particularly in the \acrshort{qcd}-enriched \glspl{SB}. As such, a requirement is placed on the ratio of the \HT formed from \glspl{jet} within $\abs{\eta} < \text{5}$ ($\HT^5$) and $\abs{\eta} < \text{2.4}$ ($\HT^{2.4}$), in OR with the difference in $\phi$ between the leading jet and \mht:
\medskip
\begin{easylist}[itemize]
    \cutflowlistprops
...
To avoid any potential horns (spikes in data and/or \acrshort{mc}) for \glspl{jet} at large pseudorapidity, we require
\medskip
\begin{easylist}[itemize]
    \cutflowlistprops
