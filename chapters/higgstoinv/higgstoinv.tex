...
\begin{easylist}[itemize]
    ...
    & $\ptjtwo > \text{40}\GeV$
    & $\HT > \text{200}\GeV$
    & $\mhtNoMu/\ptmissNoMu < \text{1.2}$
\end{easylist}

\medskip

\noindent{}Of these, events that additionally pass the ``online'' selection of the trigger requirements in the signal region (Tab.~\ref{tab:htoinv_SR_triggers}) are also recorded. The efficiency of a trigger---or collection of triggers---$\epsilon_{\mathrm{trg}}$ is defined as
\begin{equation}
    \epsilon_{\mathrm{trg}} = \frac{ s_\mathrm{offline} \cap s_{\mathrm{online}} }{ s_\mathrm{offline} }
    \label{eq:trigger_eff}
\end{equation}

where $s$ denotes the set of events that have passed the selection given by its subscript. The efficiencies for data and simulation in each data taking year are provided in Figs.~\ref{fig:htoinv_trig_effs_2016}, \ref{fig:htoinv_trig_effs_2017}, and \ref{fig:htoinv_trig_effs_2018}.
...
% Now write about how we get the scale factor for MC and uncertainties. Need to figure out whether MC is weighted by xsec x lumi (for getting SF if not for efficiency plot)
