...
Events with a \VH topology, being characterised by a dijet pair or single fat jet recoiling from the \ptvecmiss, naturally has a large \mindphiJetMet. Obtaining a multijet enriched sideband by inverting this variable was therefore not possible. The sidebands inverted in \omegaTilde also contained little \acrshort{qcd} to extrapolate from. Therefore, a single sideband was created from the 2j0b subcategory that inverts \mindphi, \omegaTilde, and the dijet mass requirement from Tab.~\ref{tab:htoinv_categories}. The latter inversion populates the sideband sufficiently with multijet events. This sideband is utilised by all \VH subcategories, as while the 1V category does not enforce the same dijet mass window, the soft drop mass requirement for the boosted \PVec-tagged \gls{jet} is a suitable proxy. Inclusion of \glspl{bjet} in the 2j1b and 2j2b subcategories reduces the multijet content of potential sidebands significantly. This is considered suitable justification for these subcategories to also utilise the sideband constructed from 2j0b.

Figs.~\ref{fig:htoinv_sb_yields_comb2016to18_ttH} and \ref{fig:htoinv_sb_yields_comb2016to18_VH} illustrate the \ptmiss distributions in the \ttH and \VH \glspl{SB} after the analysis-level selections with the full Run-2 dataset. The combined \ttH subcategories for boosted and resolved topologies are used to demonstrate the shapes and data--simulation resemblance. Background estimation is performed separately for each year due to the differing running conditions, detector configuration, and the different effects or problems seen in the data.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.23\textwidth}
        ...
        \caption{Tight double sideband --- \ttH resolved}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.23\textwidth}
        \includegraphics[width=\textwidth]{figures/region_plots/full_Run2/sideband_2/ttH_boosted.pdf}
        \caption{Loose double sideband --- \ttH boosted}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.23\textwidth}
        \includegraphics[width=\textwidth]{figures/region_plots/full_Run2/sideband_2/ttH_resolved.pdf}
        \caption{Loose double sideband --- \ttH resolved}
    \end{subfigure}
    \caption[Data--simulation comparisons of the \ptmiss distribution in the tight double sideband for the combined boosted and resolved categories for the \ttH topology, using the full Run-2 dataset]{Data--simulation comparisons of the \ptmiss distribution in the tight double \gls{SB} for the combined boosted and resolved categories for the \ttH topology, using the full Run-2 dataset.}
    \label{fig:htoinv_sb_yields_comb2016to18_ttH}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.48\textwidth]{figures/region_plots/full_Run2/VH_sideband/VH.pdf}
