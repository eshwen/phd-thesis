...
In the analysis, the purity measurement is used to estimate the \acrshort{qcd} multijet background in the \singlePhotonCr \gls{CR}, replacing the contribution from \acrshort{mc}. For each event in data that enters the region, a \acrshort{qcd} multijet pseudo-event is created with the same properties---notably \ptmiss and photon \pt. A weight is assigned to it equal to the impurity calculated from the photon's \pt. Its uncertainty (i.e., the upward variation as 1.25$\times$ the weight and the downward as 0.8$\times$) is propagated through the analysis as a systematic. The new \acrshort{qcd} background is therefore generated with the same shape as the data and weighted to represent the rate of non-prompt photons.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{c|c|c|c}
        ...
        $\dphiFj \leq \text{0.5}$ & ``Tight'' double SB & ``Loose'' double SB & $\mht/\ptmiss$ SB \\
    \end{tabular}
...
\begin{table}[htbp]
    ...
    \end{tabular}
    \caption[Definitions of the data sidebands used to determine the QCD multijet background in the signal region for the \VH category]{Definitions of the \glspl{SB} (SBs) used to determine the \acrshort{qcd} multijet background in the signal region for the \VH category.}
    \label{tab:sidebanddefs_VH}
\end{table}
