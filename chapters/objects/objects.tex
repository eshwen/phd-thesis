...
A \gls{jet} suspected to originate from a \Pbottom-quark, or ``\gls{bjet},'' is described by a nanoAOD AK4 \gls{jet} with corrected $\pt > \text{20}\GeV$ and within the acceptance of the tracker---$\abseta < \text{2.4}$ in 2016, and $< \text{2.5}$ onward. It is also required to satisfy the medium working point (a mis-tag rate of 1\,\%) from the \deepcsv algorithm~\cite{Sirunyan:2017ezt}.
