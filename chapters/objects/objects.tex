...
A \gls{bjet} is described by a nanoAOD AK4 \gls{jet} with corrected $\pt > \text{20}\GeV$, $\abseta < \text{2.5}$ (2.4 in 2016), and to satisfy the medium working point (a mis-tag rate of 1\,\%) from the \deepcsv algorithm~\cite{Sirunyan:2017ezt}. The systematic uncertainty associated with the algorithm's classification is described in Chpt.~\ref{subsec:htoinv_btagging_sfs}.
