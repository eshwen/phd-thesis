...
where the above equation is its form before the manifestation of electroweak symmetry breaking. The boson fields and their couplings are represented by $F_{\mu\nu}F^{\mu\nu}$ and $D$, respectively. Feynman slash notation is used, i.e., $\Dslash \equiv \gamma^{\mu} D_{\mu}$, with $\gamma^{\mu}$ as the Dirac \emph{gamma matrices}. The fermion fields are denoted by $\psi$, with the Yukawa coupling $y_{ij}$.\footnote{Make sure this is correct.} Finally, the Higgs field is represented with $\phi$ and potential $V(\phi)$.
